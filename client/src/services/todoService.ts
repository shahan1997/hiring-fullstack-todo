...
export const todoService = {
  ...
  updateTodo: async (id: string, input: Partial<CreateTodoInput> & { completed?: boolean }): Promise<Todo> => {
    const response = await apiClient.put<ApiResponse<Todo>>(`/todos/${id}`, input)
    if (!response.data.success) {
